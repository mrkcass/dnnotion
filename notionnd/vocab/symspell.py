...
def create_dictionary(fname):
    ...
    with open(fname) as file:
        ...
        progress = progressbar.ProgressBar(widgets=["       ",
                                                    ...
                                                    ' ', progressbar.Bar()],
                                           max_value=os.path.getsize(fname),
                                           term_width=40)
        for line in file:
...
if __name__ == "__main__":
  ...
  #try:
    corpus_path = "master.dict"
    dict_path = corpus_path + '.bin'
    if os.path.isfile(dict_path):
        [longest_word_length, dictionary] = pk.load(open(dict_path, 'r'))
    else:
        create_dictionary(corpus_path)
        pk.dump([longest_word_length, dictionary], open(dict_path, 'w'))
    #create_dictionary("master.dict")
    #except:
