...
def output_to_default(input_name):
    if re.search(r'\.[0-9a-zA-Z_\-]+$', input_name):
        output_name = re.sub(r'\.[0-9a-zA-Z_\-]+$', '_ngrm.txt', input_name)
    else:
...
def display_help():
    print ' '
    print 'ngram.py [OPTIONS] CORPUS'
    print ' '
    print '  bi and tri gram extraction and replacement using Stanford NLTK'
    print ' '
    print 'OPTIONS:'
    print '   -h, --help  : show this help screen'
    print '   -o FILENAME  :  Send ngram-ed corpus to FILENAME. the default'
    print '                  is the CORPUS with the suffix _ngrm placed'
    print '                  before the extension'
    print '   -b           :  Output bigrams. Adds project cross reference file'
    print '                  containing bigram and trigram row locations'
    print '   -t           :  Output trigrams. Adds project cross reference file'
    print '                  containing bigram and trigram row locations'
...
def parse_options(poptions):
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hbpso:", ['help'])
    ...
    if not show_help:
        for opt, arg in opts:
            if opt == '-h' or opt == '--help':
                ...
                poptions['outfile'] = arg.strip()
            elif opt == '-b':
                poptions['bigram'] = True
            elif opt == '-t':
                poptions['trigram'] = True
            elif opt == '-s':
    ...
    if show_help is False and not poptions['downloader']:
        if len(args) != 1:
            ...
            poptions['infile'] = sys.argv[len(sys.argv) - 1]
            if poptions['outfile'] == "":
                poptions['outfile'] = output_to_default(poptions['infile'])
